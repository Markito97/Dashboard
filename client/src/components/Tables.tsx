    ...
    <div className="table__arrangement">
      {tables.map((table, index) => {
        if (table.tableSize === "Small_Table") {
          return (
            <MyTable key={table.id} width="100px" onClick={takeTableId}>
              {index + 1}
            </MyTable>
          );
        } else if (table.tableSize === "Average_Table") {
          return (
            <MyTable key={table.id} width="250px" onClick={takeTableId}>
              {index + 1}
            </MyTable>
          );
        } else {
          return (
            <MyTable key={table.id} width="350px" onClick={takeTableId}>
              {index + 1}
            </MyTable>
          );
        }
